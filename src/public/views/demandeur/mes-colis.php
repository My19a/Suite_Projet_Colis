...
<div class="page-header-simple">
    ...
    foreach ($colis as $c) {
        if ((int) $c['statut_id'] === 2) $enTransit++;
        if ((int) $c['statut_id'] === 3) $enAttente++;
        if ((int) $c['statut_id'] === 4) $livres++;
    }
    ?>
    ...
    <?php if (isset($_GET['recu'])): ?>
        <div class="message message-ok">
            ...
            <div class="message-corps">Réception confirmée. Le colis est désormais marqué « Réceptionné ».</div>
        </div>
    <?php endif; ?>
    ...
    <?php if (empty($colis)): ?>
        <?= etatVide('colis', 'Aucun colis', 'Les colis de votre département apparaîtront ici.') ?>
    <?php else: ?>
            ...
            <?php foreach ($colis as $c): ?>
                ...
                <div class="carte-ligne ligne-colis">
                    ...
                    <div class="cl-fin">
                        ...
                        <?php if ((int) $c['statut_id'] === 2): ?>
                            <a class="bouton bouton-petit bouton-principal" href="/departement/colis/receptionner?id=<?= (int) $c['id_colis'] ?>" onclick="return confirm('Confirmer la réception de ce colis ?');">Confirmer la réception</a>
                        <?php endif; ?>
                    </div>
                </div>
            <?php endforeach; ?>
